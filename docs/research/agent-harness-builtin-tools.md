...
| **Document creation** | Cowork's document skills via sandboxed code execution — arguably its core work value | **Decided 2026-07-19: all of it, ASAP** — "Yes all office doc creation too ASAP. Google via MCP if available. Docx xlsx pptx locally." Roadmap item 4: PDF via PDFKit, docx/xlsx/pptx natively via OOXML/ZIPFoundation, Google Workspace via existing MCP servers only. No sandboxed code path needed — the differentiator vs every benchmark |
